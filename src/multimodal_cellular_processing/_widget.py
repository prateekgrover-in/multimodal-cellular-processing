...
class MultimodalCellularAnalysisWidget(QWidget):
    def __init__(self, viewer: "napari.viewer.Viewer"):
        ...
        group_box = QGroupBox('Multimodal Cellular Analysis')
        ...
        group_layout.addWidget(self.btn_intensity)

        self.intensity_norm = IntensityNormalization(self)
        group_layout.addWidget(self.intensity_norm)

        # Add more modules here like smoothing, segmentation, colocalization, etc.

    def find_layers(self, event):
        self.layer_names = [layer.name for layer in self.viewer.layers]

    def toggle_intensity_normalization(self):
        self.intensity_norm.setVisible(self.btn_intensity.isChecked())
